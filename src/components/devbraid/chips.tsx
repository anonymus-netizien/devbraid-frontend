...
export function CitationChip({
  ...
  return (
    <span className="mx-0.5 inline-flex items-baseline gap-1 rounded border border-hairline bg-surface px-1.5 py-0.5 font-mono text-[10px] leading-none text-muted-foreground align-middle">
      ...
      <span className="text-foreground/90 truncate max-w-[22ch]">{r}</span>
    </span>
  )
}
...
const riskColor: Record<RiskFlag, string> = {
  ...
  DEPENDENCY: 'border-warning-border bg-warning-bg text-warning',
  CI: 'border-success-border bg-success-bg text-success',
}

export function RiskChip({ flag, dense }: { flag: RiskFlag; dense?: boolean }) {
  return (
    <span
      className={cn(

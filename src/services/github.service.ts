...
export const githubService = {
  async connect(personalAccessToken: string): Promise<GitHubStatusResponse> {
    const response = await apiClient.post<ApiResponse<GitHubStatusResponse>>(
      '/github/connect',
      { personalAccessToken }
    );
  ...
  async getStatus(): Promise<GitHubStatusResponse> {
    const response = await apiClient.get<ApiResponse<GitHubStatusResponse>>(
      '/github/status'
    );
    return response.data.data;
  },
  ...
  async listRepositories(): Promise<GitRepository[]> {
    const response = await apiClient.get<ApiResponse<GitRepository[]>>(
      '/github/repos'
    );
    return response.data.data;
  },
